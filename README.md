...
## 3. RO App API — verification policy

Официальная документация RO App является единственным источником истины для API-контракта:

- Base URL проекта: `https://api.roapp.io/v2`;
- Bearer authentication через `ROAPP_API_KEY`;
- подтверждённый проектом read endpoint: `GET /orders`;
...
Полный журнал проверки: `docs/MARSEL-ROAPP-API-VERIFICATION-LEDGER-V23.md`.

### Исправление предыдущих утверждений

Структура ответа `GET /company` в текущем доступном канале документации не была независимо проверена. Поэтому поля ответа, примеры JSON и дополнительные операции для `/company` не считаются подтверждёнными и не используются в коде.

Аналогично, наличие страницы документации не является доказательством того, что конкретный endpoint разрешён текущему API-ключу. Для production-операций требуется отдельный авторизованный live probe.
...
В `app/roapp_contract.py` централизованы только проверенные ограничения API:

- максимальный документированный rate limit: 3 req/s;
- размер страницы: 50;
- официальный URL документации;
- список подтверждённых read endpoints;
- список заблокированных write-методов.

`RoAppClient` теперь не позволяет конфигурации поднять частоту выше документированного лимита. Пагинация использует централизованный размер страницы вместо отдельного числа в коде.

Тест добавлен для защиты от регрессии rate-limit policy.

## 6. Product code collision audit
...
- централизованная API policy для rate limit/page size;
- regression test для ограничения 3 req/s;
- журнал верификации API без выдуманных response fields.
...
6. Полная структура ответа `GET /company`.

## 11. Принцип дальнейшей работы
