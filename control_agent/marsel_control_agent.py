...
The implementation lives outside the ``agents`` namespace so it cannot shadow
OpenAI's ``agents`` SDK package when the repository is tested from its root.
"""
...
def build_agent():
    """Construct the OpenAI Agents SDK agent lazily to avoid namespace collisions in tests."""
    from agents import Agent, function_tool

    return Agent(
        name="MARSEL ROAPP Control Agent",
        instructions=AGENT_INSTRUCTIONS,
        tools=[function_tool(inspect_file), function_tool(run_check), function_tool(write_file)],
    )
