...
def _evidence(filename="backup_evidence.json", timestamp=None):
    data = {
        "status": "PASS", "readonly": True, "write_requests_made": 0, "ro_app_data_mutated": False,
        "source_system": "controlled-staging", "environment": "staging", "producing_job_or_run": "run-123",
        "source_version": "abc123", "producer_identity": "github-actions", "scope": "controlled-test",
        "generated_at": timestamp or datetime.now(timezone.utc).isoformat(), "sha256": "",
        "operation": "backup-test", "completion_result": "PASS", "integrity_reference": "checksum-ref",
    }
...
def test_rejects_stale_evidence(tmp_path, monkeypatch):
    ...
    assert "evidence.json: stale_evidence" in MODULE.validate_file(path)


def test_rejects_write_request(tmp_path):
    data = _evidence(); data["write_requests_made"] = 1; data["sha256"] = MODULE._canonical_sha256(data)
    path = tmp_path / "evidence.json"; path.write_text(json.dumps(data), encoding="utf-8")
    assert "evidence.json: write_requests_made_must_be_zero" in MODULE.validate_file(path)


def test_enforces_evidence_specific_requirements(tmp_path):
    data = _evidence("rollback_evidence.json"); data.pop("tested"); data["sha256"] = MODULE._canonical_sha256(data)
    path = tmp_path / "rollback_evidence.json"; path.write_text(json.dumps(data), encoding="utf-8")
    assert "rollback_evidence.json: missing_evidence_specific:tested" in MODULE.validate_file(path)


def test_rejects_non_idempotent_evidence(tmp_path):
    data = _evidence("idempotency_evidence.json"); data["idempotent"] = False; data["sha256"] = MODULE._canonical_sha256(data)
    path = tmp_path / "idempotency_evidence.json"; path.write_text(json.dumps(data), encoding="utf-8")
    assert "idempotency_evidence.json: idempotent_must_be_true" in MODULE.validate_file(path)
