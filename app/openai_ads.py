...
class OpenAIAdsClient:
    ...
    def _sanitize_source_url(source_url: str | None) -> str | None:
        ...
        # CAPI receives origin + pathname only; query strings and fragments are removed.
    ...
    async def send_order_created(
        ...
        *,
        ...
        if contents:
            event_data["contents"] = [
                item.model_dump(exclude_none=True) for item in contents
            ]
    ...
    async def try_send_order_created(self, **kwargs: Any) -> bool:
        """Best-effort conversion reporting that never blocks the business flow."""
        try:
            await self.send_order_created(**kwargs)
            return True
        except Exception:
            # Ads measurement must never make checkout/order completion fail.
            return False
