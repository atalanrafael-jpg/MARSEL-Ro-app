...
API_BASE=os.environ.get("ROAPP_API_BASE","https://api.roapp.io/v2").rstrip("/"); KEY=os.environ.get("ROAPP_API_KEY"); DOCS=os.environ.get("ROAPP_DOCS_INDEX","https://roapp.readme.io/llms.txt"); OUT="marsel-unresolved-references-v19-1.json"
if not KEY: sys.exit("ROAPP_API_KEY is not configured")
def get(url,api=False):
    h={"User-Agent":"MARSEL-UNRESOLVED-REFERENCES/19.1","Accept":"application/json,text/plain,*/*"}
...
def text(v):return v.decode("utf-8","replace") if isinstance(v,bytes) else str(v)
def json_value(v):
...
def candidate_urls(page):
    ...
    for spec in specs(page):
        servers=spec.get("servers") or [{}]; server=((servers[0].get("url") if isinstance(servers[0],dict) else None) or API_BASE).rstrip("/")
        for path,item in (spec.get("paths") or {}).items():
            if not isinstance(item,dict) or "get" not in item or re.search(r"\{[^}]+\}",path):continue
            op=item["get"] if isinstance(item["get"],dict) else {}; url=server+("/" if not path.startswith("/") else "")+path; q=[]
...
def rid(r):
    ...
    return None
def is_ref_key(k):
    lk=str(k).lower(); return lk.endswith("_id") or lk.endswith("_ids") or lk in {"customer","customerid","client","clientid","order","orderid","product","productid","service","serviceid","branch","branchid","employee","employeeid","category","categoryid","warehouse","warehouseid","invoice","invoiceid","payment","paymentid"}
def entity_hint(field,source):
    x=(str(field)+" "+str(source)).lower()
    for key in ("customer","client","order","product","service","branch","employee","category","warehouse","invoice","payment"):
        if key in x:return key
    return "unknown"
print("=== MARSEL AUDIT V19.1 / UNRESOLVED REFERENCE CLASSIFICATION / READ ONLY ===")
status,body=get(DOCS); print(f"DOCS_INDEX_HTTP={status}")
if status!=200:sys.exit(4)
links=list(dict.fromkeys(re.findall(r"https://roapp\.readme\.io/reference/[^)\s]+",text(body)))); print(f"REFERENCE_LINKS={len(links)}")
endpoints=[]
for ref in links:
    s,b=get(ref)
    if s==200:endpoints.extend(candidate_urls(b))
endpoints=list(dict.fromkeys(endpoints)); print(f"GET_PROBES={len(endpoints)}")
ids=Counter(); refs=[]; http_errors=[]
for url,path,opid,summary in endpoints:
    s,p=get(url,True)
    if not(s and 200<=s<300):http_errors.append({"path":path,"status":s}); continue
    rs=records(json_value(p))
            ...
            for k,v in r.items():
                if is_ref_key(k):
                    for val in (v if isinstance(v,list) else [v]):
                        if isinstance(val,(str,int)) and str(val):refs.append({"source_path":path,"field":str(k),"value":str(val),"entity_hint":entity_hint(k,path)})
unresolved=[r for r in refs if r["value"] not in ids]
for i,r in enumerate(unresolved,1):r.update({"reference_id":f"UR-{i:03d}","resolved_in_successful_get_lists":False,"classification":"UNRESOLVED_NEEDS_REVIEW","severity":"UNKNOWN","reason":"Value was not found in successful GET list ID index; this alone does not prove corruption.","recommended_action":"Inspect before any write operation."})
...
print(f"UNRESOLVED_REFERENCES={len(unresolved)}");print("WRITE_REQUESTS_MADE=0");print(f"REPORT={OUT}");print("RESULT=READ_ONLY; CLASSIFICATION ONLY; NO RO APP DATA CREATED, UPDATED OR DELETED")
