...
Builds an evidence-first inventory from multiple official RO App ReadMe indexes.
Only explicit API URLs/paths found in fetched documentation are eligible for
live GET probing. Operation titles are never converted into guessed paths.
"""
...
def normalize_path(raw):
    ...
    if raw.startswith(("http://", "https://")):
        p = urlparse(raw)
        if p.netloc.lower() != "api.roapp.io":
            return None
        raw = p.path
    raw = raw.split("#", 1)[0]
...
def nearby_method(text, start, end):
    ...
    matches = list(re.finditer(r"\b(GET|POST|PUT|PATCH|DELETE)\b", win, re.I))
    if not matches:
        return None
    center = start - max(0, start - 1000)
    best = min(matches, key=lambda m: abs(m.start() - center))
    return best.group(1).upper()
...
def add(store, method, path, evidence, source, detail):
    ...
    if not p:
        return
    key = (method, p)
    rank = {"OPENAPI_CONFIRMED": 4, "DOCUMENTATION_CONFIRMED": 3, "URL_CONFIRMED": 2}
    if key not in store:
        store[key] = {"method": method, "path": p, "evidence": evidence, "sources": [source] if source else [], "details": [detail] if detail else []}
        return
    item = store[key]
    ...
    if rank[evidence] > rank[item["evidence"]]:
        item["evidence"] = evidence

def extract_paths(text, source, store):
    t = html.unescape(text).replace("\\/", "/")
    for m in API_URL_RE.finditer(t):
        method = nearby_method(t, m.start(), m.end()) or "GET"
        add(store, method, m.group(0), "DOCUMENTATION_CONFIRMED", source, "explicit API URL")
    for m in HREF_RE.finditer(t):
        p = normalize_path(urljoin(source, clean(m.group(1))))
        if p:
            method = nearby_method(t, m.start(), m.end()) or "GET"
            add(store, method, p, "DOCUMENTATION_CONFIRMED", source, "HTML API link")
    for m in PATH_RE.finditer(t):
        p = normalize_path(m.group(0))
        if p:
            method = nearby_method(t, m.start(), m.end())
            if method:
                add(store, method, p, "DOCUMENTATION_CONFIRMED", source, "explicit path expression")
...
def has_placeholder(p):
    return bool(re.search(r"\{[^}]+\}|:[A-Za-z_][A-Za-z0-9_]*|<[^>]+>", p))
...
def main():
    ...
    for index_url in DOCS_INDEXES:
        status, index, elapsed, err = fetch(index_url)
        index_results.append({"url": index_url, "http": status, "elapsed_s": elapsed, "error": err})
        if status != 200:
            continue
        for m in re.finditer(r"\[([^\]]+)\]\(([^)]+/reference/[^)]+)\)", index):
            title, href = m.groups()
            u = urljoin(index_url, clean(href))
            if u not in seen:
                seen.add(u)
                links.append({"title": html.unescape(title).strip(), "url": u})
    links = links[:MAX_DOCS]
    if not links:
        print("No documentation reference pages discovered", file=sys.stderr)
        return 1
    ops, pages = {}, []
    for link in links:
        variants = [link["url"]]
        ...
        for u in dict.fromkeys(variants):
            s, body, e, er = fetch(u)
            responses.append({"url": u, "http": s, "elapsed_s": e, "error": er})
            if s == 200:
                bodies.append(body)
                extract_paths(body, u, ops)
        explicit_methods = sorted({x["method"] for x in ops.values() if link["url"] in x["sources"]})
        pages.append({"title": link["title"], "documentation_url": link["url"], "responses": responses, "content_found": bool(bodies), "title_method": title_method(link["title"]), "explicit_methods": explicit_methods, "path_evidence_count": sum(1 for x in ops.values() if link["url"] in x["sources"])})
    probes, seen_get = [], set()
    ...
    for op in sorted(ops.values(), key=lambda x: (x["path"], x["method"])):
        if op["method"] != "GET":
            continue
        if has_placeholder(op["path"]):
            probes.append({"method": "GET", "path": op["path"], "status": "NOT_PROBED", "reason": "parameterized path; no identifier guessed"})
            continue
        if op["path"] in seen_get:
            continue
        seen_get.add(op["path"])
        u = build_url(op["path"])
        s, body, e, er = fetch(u, headers=headers)
        item = {"method": "GET", "path": op["path"], "url": u, "http": s, "elapsed_s": e, "error": er}
        if s == 200:
            try:
                d = json.loads(body)
                item["json_valid"] = True
                item["json_type"] = type(d).__name__
                item["top_level_keys"] = sorted(d.keys())[:50] if isinstance(d, dict) else None
    ...
    report = {
        ...
        "sources": {"documentation_indexes": DOCS_INDEXES, "api_base": BASE, "index_results": index_results},
        "documentation": {"pages_discovered": len(links), "pages_fetched": sum(1 for p in pages if p["content_found"]), "pages_with_explicit_api_path_evidence": sum(1 for p in pages if p["path_evidence_count"] > 0)},
        "operations": sorted(ops.values(), key=lambda x: (x["path"], x["method"])),
        "get_probes": probes,
        "summary": {
            "unique_confirmed_operations": len(ops), "get_operations": sum(1 for x in ops.values() if x["method"] == "GET"),
            "non_get_operations": sum(1 for x in ops.values() if x["method"] != "GET"),
            "get_probes_attempted": sum(1 for p in probes if p.get("status") != "NOT_PROBED"),
            "get_probes_http_200": sum(1 for p in probes if p.get("http") == 200),
            "parameterized_not_probed": sum(1 for p in probes if p.get("status") == "NOT_PROBED"), "write_requests_made": 0,
        },
        "contract_state": {"completeness_claim": "NOT_ESTABLISHED", "title_method_classification_is_not_url_evidence": True, "parameterized_identifiers_guessed": False},
        "safety": {"status": "PASS", "write_requests_made": 0, "ro_app_data_mutated": False},
        "reference_pages": [{"title": p["title"], "documentation_url": p["documentation_url"], "title_method": p["title_method"], "content_found": p["content_found"], "path_evidence_count": p["path_evidence_count"]} for p in pages],
    ...
    print(f"REFERENCE_PAGES={len(links)}")
    print(f"CONFIRMED_OPERATIONS={len(ops)}")
    print(f"GET_PROBES_ATTEMPTED={report['summary']['get_probes_attempted']}")
