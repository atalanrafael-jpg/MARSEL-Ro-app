...
Canonical version 20.47. The warehouse-list contract is verified only against
...
def main():
    ...
    list_successes = [p for p in probes if p.get("documented_contract") and p.get("path") == "/v2/warehouse/" and p.get("http") == 200 and p.get("json_valid") and p.get("rows_discovered", 0) > 0 and bool(p.get("warehouse_ids_in_response"))]
