#!/usr/bin/env python3
"""MARSEL V17.2.1 — metadata-only inspection of live GET responses."""
...
OUT = "marsel-live-api-metadata-v17-2-1.json"
if not KEY:
...
def get(url, api=False):
    headers = {
        "User-Agent": "MARSEL-LIVE-AUDIT/17.2.1",
...
def refs(text):
    # Correctly match the real ReadMe reference URLs. The previous V17.2
    # version accidentally searched for literal backslashes and returned 0.
...
def specs(text):
    out = []
    # OpenAPI JSON can be embedded in fenced JSON blocks.
    ...
    for spec in specs(page):
        ...
        for path, item in (spec.get("paths") or {}).items():
            if not isinstance(item, dict) or "get" not in item:
                continue
            # V17.2.1 does not fabricate path parameters. Such endpoints
            # require an actual ID and therefore are not safe to probe here.
            if re.search(r"\{[^}]+\}", path):
                continue
...
print("=== MARSEL AUDIT V17.2.1 / LIVE GET RESPONSE METADATA / READ ONLY ===")
status, body = get(DOCS)
print(f"DOCS_INDEX_HTTP={status}")
if status != 200:
    sys.exit(4)

reference_links = refs(body.decode("utf-8", "replace"))
...
for url, path, operation_id, summary in endpoints:
    ...
    # Never store response bodies or response values.
    rows.append(
        {
            "method": "GET",
            "path": path,
...
http_errors = sum(
    row["http_status"] is not None and not 200 <= row["http_status"] < 300
    for row in rows
)
