...
OUT = os.environ.get("MARSEL_AUDIT_V6_REPORT", "marsel-audit-v6-report.json")
...
print("=== MARSEL AUDIT V6 / RO APP API / READ ONLY ===")
print(f"BASE={BASE}")
...
missing_manager = [safe_order(x) for x in all_rows if x.get("manager_id") is None]
missing_due_date = [safe_order(x) for x in all_rows if x.get("due_date") is None]
invalid_due_date = [safe_order(x) for x in all_rows if x.get("due_date") and parse_date(x.get("due_date")) is None]
...
closed_or_done_overdue = []
past_due_not_flagged = []
future_due_overdue_flag = []

for x in all_rows:
    item = safe_order(x)
    due = parse_date(x.get("due_date"))
    closed = x.get("closed_at") is not None or x.get("done_at") is not None
    if x.get("overdue") is True:
        if closed:
            item["diagnostic_reason"] = "closed_or_done_but_overdue_flag_true"
            closed_or_done_overdue.append(item)
        elif due is None:
            item["diagnostic_reason"] = "no_due_date"
        elif due < today:
            item["diagnostic_reason"] = "due_date_before_today"
        else:
            item["diagnostic_reason"] = "overdue_flag_without_past_due_date"
            future_due_overdue_flag.append(item)
    elif due is not None and due < today and not closed:
        item["diagnostic_reason"] = "past_due_not_flagged_overdue"
        past_due_not_flagged.append(item)
...
for x in all_rows:
    ...
    status_counts[str(s.get("name") or s.get("id") or "<missing>")] += 1
    type_counts[str(t.get("name") or t.get("id") or "<missing>")] += 1
    branch_counts[str(x.get("branch_id") if x.get("branch_id") is not None else "<missing>")] += 1

report = {
    "audit": "MARSEL_AUDIT_V6",
    ...
    "checks": {
        ...
        "missing_manager_id_count": len(missing_manager),
        "missing_due_date_count": len(missing_due_date),
        "invalid_due_date_count": len(invalid_due_date),
        "overdue_flag_true_count": len(overdue_flagged),
        "status_overdue_true_count": len(status_overdue_flagged),
        "closed_or_done_overdue_count": len(closed_or_done_overdue),
        "past_due_not_flagged_count": len(past_due_not_flagged),
        "future_due_overdue_flag_count": len(future_due_overdue_flag),
    },
    "missing_assignee_orders": missing_assignee,
    "missing_manager_orders": missing_manager,
    "missing_due_date_orders": missing_due_date,
    "invalid_due_date_orders": invalid_due_date,
    "overdue_orders": overdue_flagged,
    "status_overdue_orders": status_overdue_flagged,
    "closed_or_done_overdue_orders": closed_or_done_overdue,
    "past_due_not_flagged_orders": past_due_not_flagged,
    "future_due_overdue_flag_orders": future_due_overdue_flag,
    "distributions": {
        "status": dict(status_counts),
        "order_type": dict(type_counts),
        "branch_id": dict(branch_counts),
    },
    "safety": {
...
print(f"MISSING_MANAGER_ID={len(missing_manager)}")
print(f"MISSING_DUE_DATE={len(missing_due_date)}")
print(f"INVALID_DUE_DATE={len(invalid_due_date)}")
print(f"OVERDUE_FLAG_TRUE={len(overdue_flagged)}")
print(f"STATUS_OVERDUE_TRUE={len(status_overdue_flagged)}")
print(f"CLOSED_OR_DONE_OVERDUE={len(closed_or_done_overdue)}")
print(f"PAST_DUE_NOT_FLAGGED={len(past_due_not_flagged)}")
print(f"FUTURE_DUE_OVERDUE_FLAG={len(future_due_overdue_flag)}")
print(f"REPORT={OUT}")
