...
def load_plan() -> list[dict]:
    ...
    for item in plan:
        ...
        parsed = urlparse(url)
        if parsed.scheme != "https" or parsed.hostname not in ALLOWED_HOSTS or parsed.username or parsed.password or parsed.query:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="marsel-backup-") as tmp:
        out = Path(tmp)
        ...
        for item in plan:
            ...
            (out / filename).write_bytes(canonical)
        ...
        (out / "manifest.json").write_bytes(json.dumps(manifest, ensure_ascii=False, indent=2, sort_keys=True).encode("utf-8"))
        final_out.parent.mkdir(parents=True, exist_ok=True)
        if final_out.exists():
            import shutil
