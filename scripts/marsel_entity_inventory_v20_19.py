#!/usr/bin/env python3
"""MARSEL V20.34 strict READ-ONLY entity inventory.
...
collection paths, and records response evidence. No writes are executed.
Write-capable operations found in the source inventory are recorded as
blocked metadata and do not make this read-only audit fail.
"""
...
def main():
    if not KEY:
        print("ROAPP_API_KEY_missing",file=sys.stderr)
        return 2
    if not os.path.exists(INPUT):
        print(f"inventory_not_found:{INPUT}",file=sys.stderr)
        return 1
    ...
    for op in ops:
        ...
        if method in WRITE:
            blocked.append(f"{method} {op.get('path','')}")
            continue
        if method!="GET":
            continue
        p=normalize(str(op.get("path",op.get("url",""))))
        if not p or PARAM_RE.search(p):
            continue
        if p not in paths:
            paths.append(p)
    results=[]
    headers={"Authorization":f"Bearer {KEY}","Accept":"application/json","User-Agent":"MARSEL-V20.34-Readonly"}
    with httpx.Client(base_url=BASE,headers=headers,timeout=TIMEOUT,follow_redirects=False) as client:
        for path in paths:
            ...
            try:
                r=client.get(path)
                body=r.text[:1000]
                cls=("OK" if r.status_code==200 else
                     "AUTH_REQUIRED" if r.status_code in (401,403) else
                     "NOT_FOUND" if r.status_code==404 else "HTTP_ERROR")
                ...
                results.append({"path":path,"method":"GET","http":None,"classification":"TIMEOUT","error":type(e).__name__})
            except httpx.HTTPError as e:
                results.append({"path":path,"method":"GET","http":None,"classification":"NETWORK_ERROR","error":type(e).__name__})
            time.sleep(0.2)
    counts={}
    for r in results:
        counts[r["classification"]]=counts.get(r["classification"],0)+1
    report={"version":"20.34","mode":"READ_ONLY","generated_at":datetime.now(timezone.utc).isoformat(),"api_base":BASE,"source_inventory":INPUT,"source_inventory_sha256":hashlib.sha256(open(INPUT,"rb").read()).hexdigest(),"collection_paths_considered":len(paths),"results":results,"classifications":counts,"write_requests":0,"ro_app_data_mutated":False,"safe_methods_used":["GET"],"write_methods_used":[],"blocked_write_methods_detected":blocked,"blocked_write_methods_count":len(blocked),"audit_status":"PASS"}
    with open(OUTPUT,"w",encoding="utf-8") as f:
        json.dump(report,f,ensure_ascii=False,indent=2)
        f.write("\n")
    print("=== MARSEL V20.34 ENTITY INVENTORY / READ ONLY ===")
