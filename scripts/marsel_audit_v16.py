...
def is_openapi_url(url):
    ...
    return any(x in s for x in ("openapi", "swagger", "api-docs")) or (
        path.endswith((".yaml", ".yml", ".json"))
        and any(x in path for x in ("api", "spec", "swagger"))
    )
...
def response_shape(payload):
    ...
    if isinstance(payload, dict):
        keys = ("data", "items", "results", "orders", "people", "organizations", "services", "products", "employees", "locations")
        for key in keys:
...
for line in idx.text.splitlines():
    for u in links(line):
        if "/reference/" in u and u not in seen:
            seen.add(u)
            catalog.append({"url": u, "title": title_from_line(line) or u.rsplit("/", 1)[-1]})
...
for u in extract_spec_links(idx.text):
    if u not in candidate_seen:
        ...
        spec_candidates.append({"url": u, "source": "llms_index"})
...
for ref in catalog[:MAX_DOC_PAGES]:
    ...
    if r.status_code == 200:
        ...
        for u in found:
            if u not in candidate_seen:
                candidate_seen.add(u)
                spec_candidates.append({"url": u, "source": ref["url"]})
    page_results.append(rec)
...
for candidate in spec_candidates:
    r = get(candidate["url"], headers=DOC_HEADERS)
    rec = {"url": candidate["url"], "source": candidate["source"], "status": None, "valid": False}
    if isinstance(r, Exception):
        rec["error"] = str(r)
...
for spec in valid_specs:
    for path_entry in spec["paths"]:
        for method in path_entry["methods"]:
            all_ops[(method, path_entry["path"])] = {"method": method, "path": path_entry["path"], "source": spec["url"]}
...
for op in get_ops:
    if re.search(r"\{[^}]+\}", op["path"]):
        probe.append({**op, "classification": "template_not_probed", "available": None})
        continue
    url = api_url(op["path"])
    ...
    if isinstance(r, Exception):
        ...
        if r.status_code == 200:
            try:
                payload = r.json()
                rec.update({"classification": "available", "available": True, "rows_first_page": response_shape(payload)})
            except Exception:
