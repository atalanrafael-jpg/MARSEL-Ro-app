...
Only confirmed RO App contracts can satisfy the gate. Undocumented endpoints
remain diagnostic and can never produce PASS. No write operation is performed.
"""
...
def extract_rows(payload):
    ...
    def walk(value, depth=0):
        ...
        if isinstance(value, list):
            ...
            if rows and any(x.get("id") or x.get("warehouse_id") for x in rows): found.extend(rows); return
            for x in value: walk(x, depth+1)
        elif isinstance(value, dict):
            for key in preferred:
                if key in value: walk(value[key], depth+1)
            for key, child in value.items():
                if key not in preferred and isinstance(child,(dict,list)): walk(child, depth+1)
    walk(payload)
    unique=[]; signatures=set()
    for row in found:
        sig=json.dumps(row,sort_keys=True,ensure_ascii=False,default=str)
        if sig not in signatures: signatures.add(sig); unique.append(row)
    return unique
...
def main():
    ...
    if list_probe["http"] == 200 and list_probe["json_valid"] and not rows and not branch_id:
        fallback, fallback_rows=probe(f"{API_BASE}/warehouse/",{},WAREHOUSE_DOC,True)
        fallback["reason"]="documented type parameter default check"
        probes.append(fallback)
        if fallback_rows: rows=fallback_rows
    if not rows:
        for candidate in (f"{API_ROOT}/warehouse/", f"{API_BASE}/warehouses"):
            p, candidate_rows=probe(candidate,query,WAREHOUSE_DOC,False)
            p["reason"]="undocumented compatibility probe; diagnostic only"
            probes.append(p)
            if candidate_rows: rows=candidate_rows
    ids=[]
    for row in rows:
    ...
    report={"version":"20.46","mode":"READ_ONLY","result":result,"readonly":True,"write_requests_made":0,"ro_app_data_mutated":False,"official_documentation":{"warehouse_list":WAREHOUSE_DOC,"stock":STOCK_DOC},"warehouse_count":len(ids),"warehouse_ids_discovered":ids,"probes":probes,"confirmed_live_gets":confirmed,"diagnostic_only_undocumented_probes":[p for p in probes if not p.get("documented_contract")],"retry_policy":{"max_retries":MAX_RETRIES,"timeout_seconds":TIMEOUT,"retryable_http":[408,429,500,502,503,504]}}
    ...
    print(f"WAREHOUSE_CONTRACT_RESULT={result}"); print(f"WAREHOUSE_COUNT={len(ids)}"); print(f"WAREHOUSE_CONFIRMED_LIVE_GETS={len(confirmed)}"); print(f"WAREHOUSE_LIST_HTTP={list_probe['http']}"); print(f"WAREHOUSE_LIST_JSON_VALID={str(list_probe['json_valid']).lower()}"); print(f"WAREHOUSE_LIST_ROWS={list_probe['rows_discovered']}"); print("WRITE_REQUESTS_MADE=0"); print("RO_APP_DATA_MUTATED=false")
    return 0
