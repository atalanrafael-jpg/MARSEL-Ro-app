...
def main() -> int:
    ...
    if any(method in registry_text for method in FORBIDDEN_WRITE_METHODS): fail("canonical READ-ONLY API registry contains a write method")
    if "/v2/v2" in workflow_text: fail("duplicated /v2/v2 API base detected in workflow")
    for marker in ("write_requests_made", "ro_app_data_mutated", "readonly"):
        if marker not in workflow_text: fail(f"workflow safety marker missing: {marker}")
    if "Production mutations remain disabled" not in arch_text and "Production WRITE remains disabled" not in arch_text: fail("architecture does not explicitly keep production WRITE disabled")
